...
说法当作确凿事实之前，请先核对来源，尤其是打算公开引用的场合。如果发现错误，欢迎
[提交一个报错 issue](https://github.com/DOTfei/Learn-From-Failure/issues/new?template=case-error.yml)
或直接提 PR。
...
**知道一个该收录的案例，或者想给已有案例补充更好的来源，但不想自己写完整文件？**
[提交一个案例线索](https://github.com/DOTfei/Learn-From-Failure/issues/new?template=new-case.yml)
或 [补充一个来源](https://github.com/DOTfei/Learn-From-Failure/issues/new?template=add-source.yml) ——
把你知道的信息填进去就行，不需要写 PR 或 Markdown。

**想自己改一个来源或改点小东西，但电脑上没配 git？** 直接在 GitHub 网页上打开那个案例文件
（比如 `cases/enron.md`），点右上角的 ✏️ 铅笔图标编辑，然后提交 —— GitHub 会自动帮你
fork 仓库并开一个 PR，完全不需要克隆仓库到本地。

**想自己在本地写一个完整案例？** 参考 [CONTRIBUTING.md](CONTRIBUTING.md)（英文）。简单说：用 `cases/_template.md` 做模板，
